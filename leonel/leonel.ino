...
int velocidadeBusca  = 180;
int velocidadeRecuo  = 220;

int distanciaMaximaAtaque = 100;//50
int distanciaMinimaAtaque = 35;//20

float distanciaFrenteDireita = 999;//51
float distanciaFrenteEsquerda = 999;//51
float distanciaTraseiraDireita = 999;//51
float distanciaTraseiraEsquerda = 999;//51
...
void setup() {
  ...
  pinMode(TRIG_FE, OUTPUT);
  pinMode(ECHO_FE, INPUT);

  pinMode(TRIG_TD, OUTPUT);
  pinMode(ECHO_TD, INPUT);

  pinMode(TRIG_TE, OUTPUT);
  pinMode(ECHO_TE, INPUT);
...
// =====================================
float medirDistanciaUltrassonico(int pinoTrigger, int pinoEcho) {
  ...
  long tempo = pulseIn(pinoEcho, HIGH, 15000);//6000

  if (tempo > 0) {
    return tempo * 0.01715;
  } else {
    return 999;//51
  }
}
...
void lerTodosUltrassonicos() {
  distanciaFrenteDireita = medirDistanciaUltrassonico(TRIG_FD, ECHO_FD);
  distanciaFrenteEsquerda = medirDistanciaUltrassonico(TRIG_FE, ECHO_FE);
  distanciaTraseiraDireita = medirDistanciaUltrassonico(TRIG_TD, ECHO_TD);
  distanciaTraseiraEsquerda = medirDistanciaUltrassonico(TRIG_TE, ECHO_TE);
}
